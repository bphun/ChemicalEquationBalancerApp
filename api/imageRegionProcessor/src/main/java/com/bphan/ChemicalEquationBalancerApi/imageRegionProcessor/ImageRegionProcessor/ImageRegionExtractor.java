...
public class ImageRegionExtractor {
  ...
  @Autowired private ImageTransceiver imageTransceiver;
  ...
  public Map<String, List<ImageTransformerResponse>> createRegionImages(
      List<ImageRegion> regions, boolean includeRegionInResponse) {
    Map<String, List<ImageTransformerResponse>> response = new HashMap<>();

    // This is going to require a lot of memory once there
    // are alot of images
    ...
    regions
        ...
        .forEach(
            region -> {
              ...
              ImageTransformerResponse transformerResponse =
                  new ImageTransformerResponse(
                      success ? "success" : "error", "", requestId, region.getId(), regionImageUrl);
    ...
    logger.log(
        ...
        "Extracted "
            ...
            + "ms. Failed to extract "
            + (numRegions - numResponseRegions)
            + " regions.");

    return response;
  }

  public BufferedImage createRegionImage(
      String requestId, BufferedImage image, ImageRegion region) {
    ...
    if (parentImageWidth > 0
        ...
        && regionHeight > 0) {
      resultImage =
          imageOperations.scale(image, scaleOriginX, scaleOriginY, regionWidth, regionHeight);
      if ((regionWidth > regionHeight) && (regionWidth > 600 || regionHeight > 100)) {
        resultImage =
            imageOperations.resize(
                resultImage,
                (int) (regionWidth / widthScaleFactor),
