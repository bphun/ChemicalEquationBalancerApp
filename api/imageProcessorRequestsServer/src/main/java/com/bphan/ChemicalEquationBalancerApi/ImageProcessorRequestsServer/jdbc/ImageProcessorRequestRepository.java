...
public class ImageProcessorRequestRepository {
  ...
  public ApiResponse uploadRequestInfo(
      String requestId,
      String s3ImageUrl,
        ...
        int changedRows =
            jdbcTemplate.update(
                "INSERT INTO ImageProcessorRequestInfo"
                    ...
                    + "userInputtedChemicalEquationString)"
                    + " VALUES (?, ?, ?, ?, ?)",
                requestId,
                s3ImageUrl,
                requestStartTime,
  ...
  public List<StoredRequestInfo> getRequestList() {
    return jdbcTemplate.query(
        "SELECT * FROM ImageProcessorRequestInfo ORDER BY gcpRequestStartTimeMs ASC",
        (resource, rowNum) ->
            new StoredRequestInfo(
                ...
                getLabelingStatusForRequest(
                    resource.getString("id"), resource.getString("labelingStatus"))));
  }
  ...
  public StoredRequestInfo getRequestWithId(String requestId) {
    return jdbcTemplate
        .query(
            "SELECT * FROM ImageProcessorRequestInfo " + "WHERE id='" + requestId + "'",
            (resource, rowNum) ->
                new StoredRequestInfo(
                    resource.getString("id"),
                    resource.getString("s3ImageUrl"),
  ...
  public int numRegionsForRequest(String requestId) {
    ...
    jdbcTemplate.query(
        "SELECT * FROM ChemicalEquationRegions WHERE imageProcessorRequestInfoId=\'"
            + requestId
            + "\';",
        countCallback);
  ...
  public ApiResponse updateUserInputtedChemicalEquationString(String id, String value) {
    return doValueUpdate(
        id, "ImageProcessorRequestInfo", "userInputtedChemicalEquationString", value);
  }
  ...
  public ApiResponse updateGcpIdentifiedChemicalEquationString(String id, String value) {
    return doValueUpdate(
        id, "ImageProcessorRequestInfo", "gcpIdentifiedChemicalEquationString", value);
  }
  ...
  private ApiResponse addRegion(ImageRegion region) {
    ...
    try {
      String query =
          "INSERT INTO ChemicalEquationRegions "
      ...
      int changedRows =
          jdbcTemplate.update(
              query,
              region.getId(),
              region.getRequestInfoId(),
              region.getRegionClass(),
              region.getOriginX(),
              region.getOriginY(),
              region.getWidth(),
              region.getHeight(),
              region.getViewportWidth(),
              region.getViewportHeight(),
              region.getParentImageWidth(),
              region.getParentImageHeight(),
              region.tagStr(),
              region.getRegionClass(),
              region.getOriginX(),
              region.getOriginY(),
  ...
  public ApiResponse updateRegions(RegionDiff regionDiff) {
    ...
    updateLabelingStatusForRequest(
        requestId, numRegionsForRequest(requestId) > 0 ? "LABELED" : "INCOMPLETE");
  ...
  public List<ImageRegion> getRegionsForRequest(String requestId) {
    return jdbcTemplate.query(
        "SELECT * FROM ChemicalEquationRegions"
            + " WHERE imageProcessorRequestInfoId='"
            + requestId
            + "'",
        (resource, rowNum) ->
            new ImageRegion(
                ...
                resource.getString("regionClass"),
                resource.getString("s3ImageUrl"),
                resource.getString("equationStr"),
                resource.getInt("originX"),
                resource.getInt("originY"),
                resource.getInt("width"),
                resource.getInt("height"),
                resource.getInt("viewportWidth"),
                resource.getInt("viewportHeight"),
                resource.getInt("parentImageWidth"),
                resource.getInt("parentImageHeight"),
          ...
          jdbcTemplate
              .query(
                  "SELECT * FROM ImageProcessorRequestInfo WHERE "
                      ...
                      + " ORDER BY "
                      + "gcpRequestStartTimeMs ASC LIMIT 0,1;",
  ...
  public ApiResponse deleteRequest(String requestId) {
    ...
    if (requestId != null && requestId.trim().length() > 0) {
      try {
        int rowsAffected =
            jdbcTemplate.update("DELETE FROM ImageProcessorRequestInfo WHERE id = ?", requestId);
        if (rowsAffected > 0) {
          response = new ApiResponse("success", "");
        } else {
          response = new ApiResponse("error", "SQL error");
  ...
  private ApiResponse doValueUpdate(
      String requestId, String tableName, String valueId, Object value) {
    ApiResponse response;
    ...
    if (requestId != null && requestId.trim().length() > 0) {
      try {
        int changedRows =
            jdbcTemplate.update(
                "UPDATE " + tableName + " SET " + valueId.toString() + " = ? " + "WHERE id = ?",
                value,
                requestId);
        if (changedRows > 0) {
          response = new ApiResponse("success", "");
